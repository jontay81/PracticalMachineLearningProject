...
#PROBABLY TEST OTHER MODELS THAN RF

```{R}
library(pls)

pls.model<-train(classe~.,data=rf.train,method="pls",
                trControl=trainControl(method="loocv"),
                prox=TRUE,allowParallel=TRUE)

print(pls.model$finalModel)
```

```{R, echo=TRUE}
pls.model.fit <- predict(pls.model, newdata = rf.test)
confusionMatrix(data = pls.model.fit, rf.test$classe)
```